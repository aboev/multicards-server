...
class Question
  ...
  def self.make_random(qtype, set_id, question_id)
    if qtype == QTYPE_MULTI_CHOICE
      return make_multichoice_qquestion(set_id, question_id)
    elsif qtype == QTYPE_DIRECT_INPUT
      question = make_directinput_qquestion(set_id, question_id)
      return question if question != nil
      return make_random(QTYPE_MULTI_CHOICE)
    end
  end

  def self.make_multichoice_qquestion(set_id, question_id)
    return if set_id == -1
  ...
  def self.make_directinput_qquestion(set_id, question_id)
    return if set_id == -1
